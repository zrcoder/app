//go:generate go run _gen/html.go
//go:generate go run _gen/scripts.go
//go:generate go fmt

// Package app is a package to build progressive web apps (PWA) with Go/Go+
// programming language and WebAssembly.
// It uses a declarative syntax that allows creating and dealing with HTML
// elements only by using Go/Go+, and without writing any HTML markup.
...
func navigate(d Dispatcher, rawURL string) {
	...
	if err != nil {
		panic(errors.New("navigating to URL failed").
			Tag("url", rawURL).
			Wrap(err))
	}
	navigateTo(d, u, true)
