...
func (s *store) Set(key string, v interface{}, opts ...StateOption) {
	...
	if state.IsPersistent {
		if err := s.setPersistent(key, state.IsEncrypted, state.ExpiresAt, v); err != nil {
			panic(errors.New("persisting state failed").
				Tag("state", key).
				Wrap(err))
			return
		}
	...
	if state.IsBroadcasted {
		if err := s.broadcast(key, v); err != nil {
			panic(errors.New("broadcasting state failed").
				Tag("state", key).
				Wrap(err))
			return
		}
		...
		s.disp.Dispatch(Dispatch{
			...
			Function: func(ctx Context) {
				...
				if err := storeValue(o.receiver, v); err != nil {
					panic(errors.New("notifying observer failed").
						Tag("state", key).
...
func (s *store) Get(key string, recv interface{}) {
	...
	if err != nil {
		panic(errors.New("getting state value failed").
			Tag("state", key).
			Wrap(err))
	}
}
