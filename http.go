...
func (h *Handler) makeAppJS() []byte {
	...
	for k, v := range h.Env {
		if err := os.Setenv(k, v); err != nil {
			panic(errors.New("setting app env variable failed").
				Tag("name", k).
				Tag("value", v).
				Wrap(err))
		}
...
func (h *Handler) serveProxyResource(resource ProxyResource, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		panic(errors.New("getting proxy static resource failed").
			Tag("url", u).
	...
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		panic(errors.New("reading proxy static resource failed").
			Tag("url", u).
			Tag("proxy-path", resource.Path).
			Tag("static-resource-path", resource.ResourcePath).
			Wrap(err),
